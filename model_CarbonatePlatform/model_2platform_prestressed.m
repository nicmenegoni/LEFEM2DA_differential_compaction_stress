...
APPLY2MAT=3;%material where the body force is applied; 1= Basin, 2=1st Platform; 3 =2nd platform
...
E1 = 0.1e9;%GPa
nu1 = 0.15;
D1 = elasticitymatrix(E1, nu1,1);%Elasticity matrix
rho1=0.22e4;%Kg/m^3

%Material parameters
E2 = 0.1e11;%GPa
nu2 = 0.3;
D2 = elasticitymatrix(E2, nu2,1);%Elasticity matrix
rho2=0.271e4;%Kg/m^3
...
if apply_prestress ==1
    old_element=find(material==1 | material==2);   
    for i=1:size(old_element,1)     
        nodes = t(old_element(i), :);
        dofs = reshape([2 * nodes - 1; 2 * nodes], 1, 2 * numel(nodes));
        F(dofs) = reshape([Snew(nodes,1)'; Snew(nodes,2)'], 1, 2 * numel(nodes));
...
save('slm2d_2plat_mesh','vert','p','p_new','tria','tnum','material','etri','number_of_nodes')
save('slm2d_2plat_results','p_new','S','Sp', 'Taumax','teta2p', 'teta2s', 'tetap','Eps','displacements')