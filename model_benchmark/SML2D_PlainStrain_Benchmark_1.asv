...
%Material 2 - Antecedent Carbonate platform (compactable)
E2 = 0.1e11;%GPa
nu2 = 0.3;
D2 = (E2 / ((1+nu2)*(1 - 2*nu2))) * ...
    [ 1-nu2 nu2            0;
    nu2  1-nu2            0;
    0  0 (1 - 2*nu2) / 2];%Flexural rigidity or elasticity matrix?
rho2=0.5e4;%Kg/m^3
...
for e = 1 : number_of_elements
    nodes = t(e, :);
    %I = p(2, nodes) == max(p(2, :));%This should apply the force on the last x value?
    if consider_gravity==0
        I = p(2, nodes) == 400;%This should apply the force on the last x value?
        if( sum(I) == 2)
            t_Neumann = [t_Neumann; nodes(I)];
        end
    else % if gravity is considered, at each node it will be applied a force
        t_Neumann = [];
    end
end
%% Forces definition
...
if size(t_Neumann, 1)>0
    for element = 1 : size(t_Neumann, 1)
        ...
        if mean(p(1,t_Neumann(element)))<600%overbunden of the flat-part of platform
            yForce= -g*rho(2)* (100);%
            q = [0; yForce];
            Fe = H_mean' * q * length_of_element;
        elseif mean(p(1,t_Neumann(element)))>=600 && mean(p(1,t_Neumann(element)))<773%overbunden of the inclined-part of platform
            yForce= -g*rho(2)* ((-0.578*p(1,t_Neumann(element))+846.8)-400);
            q = [0; yForce];
            Fe = H_mean' * q * length_of_element;
        elseif mean(p(1,t_Neumann(element)))>=773
            yForce= 0;
            q = [0; yForce];
            Fe = H_mean' * q * length_of_element;
        end
...
%% Dirichlet boundary (This define the points with no movement?)
for nodes = 1 : size(vert, 1)
    %nodes = etri(element, :);
    if p(1,nodes) == 1000 && p(2, nodes)== 0%Fixed Boundary
    ...
    elseif p(2, nodes) == 0  %Roller Boundary (blocked onto y)
        ...
        F(dofs(2,:)) = 0;
%     elseif p(1, nodes) == 0 && p(2, nodes)>0%Roller Boundary?
%         dofs = [2 * nodes - 1; 2 * nodes];
%         K(dofs(1,:), :) = 0;
